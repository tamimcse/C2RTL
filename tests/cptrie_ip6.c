              ...
              if (c64_bitmap[idx] & (MSK >> off)) {
                ...
                if (c72_bitmap[idx] & (MSK >> off)) {
                  ...
                  if (c80_bitmap[idx] & (MSK >> off)) {
                    ...
                    idx = idx_sail >> 6;
                    off = idx_sail & 63;
                    if (c88_bitmap[idx] & (MSK >> off)) {
                      ck_idx = c88_popcnt[idx] + POPCNT64(c88_bitmap[idx] >> (64 - off));
                      ...
                      if (c96_bitmap[idx] & (MSK >> off)) {
                        ...
                        idx = idx_sail >> 6;
                        off = idx_sail & 63;
                        if (c104_bitmap[idx] & (MSK >> off)) {
                          ck_idx = c104_popcnt[idx] + POPCNT64(c104_bitmap[idx] >> (64 - off));
                          idx_sail = (ck_idx << 8) + ((ip2 >> 16) & 0XFF);
                          idx = idx_sail >> 6;
                          off = idx_sail & 63;
                          if (c112_bitmap[idx] & (MSK >> off)) {
                            ck_idx = c112_popcnt[idx] + POPCNT64(c112_bitmap[idx] >> (64 - off));
                            idx_sail = (ck_idx << 8) + ((ip2 >> 8) & 0XFF);
                            idx = idx_sail >> 6;
                            off = idx_sail & 63;
                            if (c120_bitmap[idx] & (MSK >> off)) {
                              ck_idx = c120_popcnt[idx] + POPCNT64(c120_bitmap[idx] >> (64 - off));
                              idx_sail = (ck_idx << 8) + (ip2 & 0XFF);
                              idx = idx_sail >> 6;
                              off = idx_sail & 63;
                              if (b128_bitmap[idx] & (MSK >> off)) {
                                n_idx = b128_popcnt[idx] + POPCNT64(b128_bitmap[idx] >> (64 - off));
                                return leafN[n_idx];
                              }
                            } else if (b120_bitmap[idx] & (MSK >> off)) {
                              n_idx = b120_popcnt[idx] + POPCNT64(b120_bitmap[idx] >> (64 - off));
                              return leafN[n_idx];
                            }
                          } else if (b112_bitmap[idx] & (MSK >> off)) {
                            n_idx = b112_popcnt[idx] + POPCNT64(b112_bitmap[idx] >> (64 - off));
                            return leafN[n_idx];
                          }
                        } else if (b104_bitmap[idx] & (MSK >> off)) {
                    ...
                    } else if (b88_bitmap[idx] & (MSK >> off)) {
